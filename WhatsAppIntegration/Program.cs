...
namespace WhatsAppIntegration_RTL
{
    public class Program
    {
        ...
        public async System.Threading.Tasks.Task SendWhatsAppMsg(string SendMsgURL)
        {
            ...
            string cmdUserDtl = @"select distinct ID as UserID,Employee_Code,Employee_Name ,
                StoreID,RoleID,ManagerID,Emp_MobileNo,Emp_Channel,CASE WHEN e.RoleID IN (SELECT a.RoleID FROM RoleMaster a   
				JOIN RoleChannelAccess_Map_tbl b ON a.RoleID = b.RoleID        
                WHERE b.AttendanceMark = 1 AND b.ChannelCode = e.Emp_Channel) THEN 1 ELSE 0 END AS AttendenceFlag from EmployeeMaster e
                where SyncStatus=1 and LTRIM(RTRIM(Emp_Channel))='RTL'   and (Emp_MobileNo is not null or Emp_MobileNo<>'')";// and ID=15756,15565";
                        ...
                        try
                        {
                            ...
                            using (HttpClient clientOpt = new HttpClient())
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        if (dsDtl.Tables[0].Rows[i]["AttendenceFlag"].ToString() == "1") // Promoter
                                        {
                                            //spname = "[proc_WhatsappMSG_Promoter_16/10/2023]"; // next morning 8am

                                            spname = "[proc_WhatsappMSGRTL_Promoter_7PM]"; // Same Day 7pm
                                            ...
                                            parameter[1] = new SqlParameter("@StoreID", string.IsNullOrEmpty(dsDtl.Tables[0].Rows[i]["StoreID"].ToString()) ? "" : dsDtl.Tables[0].Rows[i]["StoreID"].ToString());
                                            ...
                                            if (dsPromoter != null && dsPromoter.Tables.Count > 0 && dsPromoter.Tables[0].Rows.Count > 0)
                                            {
                                                ...
                                                //msgObj.msg = "Channel+%3A+RTL%0ADaily+Tracker+%28P%29+for+" + dsPromoter.Tables[0].Rows[0]["CreatedDate_dd"].ToString() + "%2F" + dsPromoter.Tables[0].Rows[0]["CreatedDate_mm"].ToString() + "%0AAttendance+%3A+" + dsPromoter.Tables[0].Rows[0]["AttendanceStatus"].ToString() + "%0AFTD+Sales+%3A+" + dsPromoter.Tables[0].Rows[0]["FTDSales"].ToString() +
                                                //    "%0AMTD+%28Tgt%2FAch%2FAch%25%29+%3A+" + dsPromoter.Tables[0].Rows[0]["MTDTarget"].ToString() + "/" + dsPromoter.Tables[0].Rows[0]["MTDSales"].ToString() + "/" + dsPromoter.Tables[0].Rows[0]["MTDAchivementPercent"].ToString() + "%0A%0AThis+is+an+autogenerated+message.+Kindly+do+not+replay+back.";
                                        ...
                                        { //Manager
                                            ...
                                            spname = "[proc_WhatsappMSGRTL_Manager_7PM]"; // Same Day 7pm
                                            ...
                                            if (dsManager != null && dsManager.Tables.Count > 0 && dsManager.Tables[0].Rows.Count > 0)
                                            {
                                                ...
                                                msgObj.msg = "Channel+%3A+RTL%0ADaily+Tracker+%28M%29+for+" + dsManager.Tables[0].Rows[0]["CreatedDate_dd"].ToString() + "%2F" + dsManager.Tables[0].Rows[0]["CreatedDate_mm"].ToString() +
                                                    ...
                                                    "%0AFTD+Sales+%3A+" + dsManager.Tables[0].Rows[0]["FTDSales"].ToString() + "%0AMTD+%28Tgt%2FAch%2FAch%25%29+%3A+" + dsManager.Tables[0].Rows[0]["MTDTarget"].ToString() + "%2F" + dsManager.Tables[0].Rows[0]["MTDSales"].ToString() +
                                                    "%2F" + dsManager.Tables[0].Rows[0]["MTDAchivementPercent"].ToString() + "%0A%0AThis+is+an+autogenerated+message.+Kindly+do+not+replay+back.";
